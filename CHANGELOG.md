...
## v6.9.0 — 2026-08-19 · จัดพอร์ตเหลือตารางเดียว + เลิกทิ้งเงินค้าง

### Changed

- **หน้าจัดพอร์ตเหลือตารางเดียวปุ่มเดียว** — ตัดช่องกรอกเงินเติมกับปุ่มคำนวณแยกทิ้ง เหลือปุ่ม "บันทึก + คำนวณใหม่" ปุ่มเดียว แก้จำนวนหุ้นกับเงินสดในตารางแล้วกด แผนซื้อขึ้นเองทันที
- **เงินสดคือช่องหนึ่งในสัดส่วน 100%** ไม่ใช่เงินสำรองแยก เงินที่ซื้อได้ = เงินสดส่วนที่เกินเป้าของตัวเอง
- **แผนซื้อติดมากับการบันทึก** ไม่ต้องยิงคำขอรอบสอง — กดปุ่มครั้งเดียวทั้งสองตารางขยับพร้อมกัน
- **บรรทัดบนตารางแผนซื้อ** บอกตรงๆ ว่าซื้อได้กี่บาท หรือบอกเหตุผลถ้ายังซื้อไม่ได้

### Fixed

- **เงินค้างเพราะซื้อไม่ครบชุด 100 หุ้น** — ของเดิมแบ่งเงินตามสัดส่วนช่องว่างก่อนแล้วค่อยปัดลงเป็นชุด เงินที่แบ่งให้หุ้นแพงที่ซื้อไม่ครบชุดจะตกไปกองเป็นเงินสดเงียบๆ (เติม 20,000 ค้างเปล่า 9,069 = 45% · เติม 50,000 ค้าง 19,972 = 40% · เติม 100,000 ค้าง 14,214 = 14%). ตัวใหม่ไล่ซื้อทีละชุดให้ตัวที่ห่างเป้าที่สุดและซื้อไหว แล้วคิดระยะห่างใหม่ทุกรอบ เงินจึงไหลไปตัวที่ซื้อได้จริง
- **ราคาไม่มาแล้วโชว์เป็นมูลค่า 0 พร้อมป้าย "ขาด"** — แยกไม่ออกจากหุ้นที่ขาดเป้าจริง ตอนนี้ขึ้น "ราคาไม่มา" ตรงๆ และไม่ถูกนับเข้าสัดส่วน
- **แถวเงินสดในตารางแผนซื้อเด้งขึ้นบนสุด** เพราะโดนเรียงตามจำนวนเงินรวมกับหุ้น ตอนนี้ต่อท้ายเสมอ พร้อมบอกเงินสดหลังซื้อคิดเป็นกี่เปอร์เซ็นต์
- **ไฟล์เปิดเซิร์ฟเวอร์ชี้โฟลเดอร์ผิดชื่อ** (ค้างจากตอนเปลี่ยนชื่อโฟลเดอร์โปรเจกต์)

### Removed

- **บล็อกนอกแผน LH/TISCO และสัญญาณ LH ทั้งหมด** — จบเรื่องแล้ว ถอดออกทั้งหน้าจอ ฝั่งเซิร์ฟเวอร์ ตัวดึงราคารายวัน ตัวย้ายพอร์ต และไฟล์พอร์ตทั้ง 3 ใบ
- **ทางเรียกคำนวณเงินเติมแยก** — แผนซื้อมากับข้อมูลพอร์ตแล้ว

### Verify

- ชุดทดสอบ 9 ตัวผ่านหมด รวมตัวกันบั๊กเงินค้างกลับมา ตัวทดสอบความนิ่งของผลลัพธ์ และตัวทดสอบเพดานรอบคำนวณ
- พอร์ตจริง: เงินสด 419,984 (4.6%) ต่ำกว่าเป้า 5% จึงขึ้นว่ายังไม่ต้องซื้อ — ถูกต้อง
- จำลองเงินสด 719,984: ซื้อได้ 248,579.8 → BBL 700 · SISB 6,800 · TOA 1,800 · SECURE 1,900 รวม 247,890 เงินสดเหลือ 472,094 = 5.01% ตรงเป้า
- ความห่างจากเป้ารวมทั้งพอร์ตหลังจัด: 19.10 → 1.09 และหุ้นที่ขาดทุกตัวเหลือห่างเท่ากันหมด

## v6.8.2 — 2026-07-27 · แก้สูตรเติมเงินให้ซื้อได้จริง

### Fixed

- **ราคาหายแล้วคำนวณพอร์ตผิด** — ถ้าหุ้นในแผนไม่มีราคา ระบบจะหยุดและให้ดึงราคาใหม่ แทนการถือว่ามูลค่าเป็นศูนย์แล้วเทเงินไปหุ้นตัวนั้น
- **จำนวนหุ้นซื้อไม่ได้จริง** — แผนซื้อปัดเป็นชุดละ 100 หุ้น ยอดบาทของหุ้นตรงกับจำนวนหุ้นคูณราคา และเงินที่เหลือกลับเข้าเงินสด
- **เปอร์เซ็นต์ขาดผิดฐาน** — เปลี่ยนเป็นช่องว่างเทียบมูลค่าพอร์ตหลังเติมเงิน ไม่ขึ้นหลักพันเปอร์เซ็นต์จากการหารด้วยเงินก้อนใหม่
- **น้ำหนักพอร์ต 4 ตัวรวม 99.9%** — ชดเชยเศษปัดที่ตัวสุดท้ายให้น้ำหนักรวม 100.0% ทุกกรณี
- **หน้าผลลัพธ์ desktop/mobile** — แยกหุ้นที่เงินไม่ถึง 100 หุ้นออกจากหุ้นเกินหรือตรงเป้า และแสดงเงินเหลือจริงในช่องเงินสด

### Verify

- ชุดทดสอบสูตร 4 กรณีผ่านทั้งหมด
- พอร์ต A/B/C ผ่านเงินเติม 10,000 / 100,000 / 1,000,000 บาท: หุ้นทุกตัวหาร 100 ลงตัว ยอดซื้อหุ้นตรงราคา และหุ้นรวมเงินสดเท่ากับเงินเติม

## v6.8.1 — 2026-06-25 · เก็บกวาด portfolio.json + แก้ cache ราคาว่างค้าง

### Fixed

- **ดึงราคาค้าง** — cache ราคาปิด (EOD) ที่ดึงก่อน SETSMART ปล่อยข้อมูล = ไฟล์ว่างค้างถาวร รอบถัดไปอ่านว่างแล้วไม่ยอมดึงใหม่ · fix: cache ว่าง = ถือว่ายังไม่มี ลบไฟล์ค้าง + ดึงใหม่ (ไม่ save ถ้าดึงได้ว่าง)

### Changed

- **`data/portfolio.json` (legacy fallback A) เลิก track git** — ข้อมูลพอร์ตจริงอยู่ที่ `portfolios/{A,B,C}.json` (gitignored กันส่วนตัว) แล้ว · ไฟล์เดิมยังอยู่ใน disk ใช้งานปกติ ไม่ต้องกรอกใหม่
