...
**Status:** FINAL v1.2 — Thai-pragmatic threshold relaxation (2026-05-18)
**Version:** 1.2
**Created:** 2026-05-16
...
|------|-------|-----------------------------------|
| ธุรกิจนิ่ง (STABLE_BUSINESS) | 4 | sector ใน STABLE/STABLE_UTILITY + eps_cv_10y ≤40% (v1.2: loosened from 30%) |
| ผ่านวิกฤตได้ (RESILIENT_THROUGH_CRISIS) | 6 | crisis_2011_drop ≥−55% + ocf_2011 >0 + crisis_2020_drop ≥−55% + ocf_2020 >0 (v1.2: loosened from −40%) |
...
| EPS swing | คะแนน |
|-----------|-------|
| 30-40% | 0 (base only) — v1.2 shifted right |
| 20-30% | +1 |
| <20% | +2 |
...
| CYCLICAL_BUSINESS | −3 | Stage 5 | ch6 daily-use vs disruption — sector วัฏจักรไม่เหมาะ DCA 10-20 ปี แต่ไม่ห้าม (v1.2: lighter -3, Thai-pragmatic) |
...
5. **CYCLICAL_BUSINESS = penalty −3** — Niwes ch6 daily-use vs disrupt + Phase 4 base 0 อยู่แล้ว (ไม่ disqualify เพราะ cyclical ปกติ ≠ disrupted). v1.2: เบาลงจาก −5 เพื่อสะท้อนตลาดไทยที่ quality stock หลายตัวมี cyclical exposure
...
- 5 ป้าย penalty (−15 to −3)
- 3 ป้าย no-action (MIXED_STABILITY + NO_MOAT + INTERMITTENT — base 0 อยู่แล้ว)

**Test impact (5 หุ้น sample):**
- SCC (DIVIDEND_SHRINKING + YIELD_TRAP + CYCLICAL): penalty rolls = −10 − 15 − 3 = −28 → anchor = max(0, ~23 − 28) = **0** (v1.2: CYCLICAL −3 instead of −5)
    ...
    'anchor': {
        ...
        'long_hold': {
            ...
            'bonus_metrics': {
                'eps_cv_10y': {
                    'steps': [
                        (0.40, 0),  # base only — gate threshold (v1.2: shifted from 0.30)
                        (0.30, 1),
                        (0.20, 2),
                    ],
...
- 2026-05-18 v1.2: Thai-pragmatic threshold relaxation (A+B+C combined)
  - CYCLICAL_BUSINESS penalty: -5 -> -3 (lighter — reflect Thai market reality)
  - STABLE_BUSINESS: eps_cv gate 0.30 -> 0.40 (cyclical with steady earnings can qualify)
  - RESILIENT_THROUGH_CRISIS: crisis drop gate -0.50 -> -0.55 (more stocks pass)
  - CASHFLOW_DETERIORATING: skip Banks/Finance/Insurance + Property Development/REIT (lumpy cash flow legitimate)
  - FAKE_PROFIT exclusion extended to Property sectors as well
  - eps_cv_10y bonus steps shifted: (0.30,0)(0.24,1)(0.15,2) -> (0.40,0)(0.30,1)(0.20,2)
