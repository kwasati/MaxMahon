...
- `data/portfolios/{A,B,C}.json` — **3 พอร์ตแยกไฟล์ (v6.8.0)** single source ต่อพอร์ต (name/targets/holdings/cash/off_plan/lh_triggers/meta). A = ของเดิม, B/C โครงหุ้น+เป้าเดียวกัน (จำนวน 0). gitignored — runtime ส่วนตัว (privacy). _legacy `data/portfolio.json` ยังอยู่เป็น fallback A ก่อน archive_
