...
- `scripts/portfolio_state.py` — **พอร์ตจริง (v6.7.0 redesign + v6.8.0 multi-portfolio)**: load/save/`build_state()`/`rebalance_topup()` รับ `portfolio_id` (A/B/C, default A + fallback `portfolio.json` เดิมกัน server พังก่อน migrate) อ่านจาก `data/portfolios/{id}.json` · `build_state` คืน positions/cash/summary + `price_as_of` (newest mtime ใน price_cache) + `name`. Used by `/api/portfolio/{state,holdings,topup,lh-signals}` (ทุกตัวรับ `?pf=`). คนละ feature กับ portfolio_builder (นั่น = สร้างพอร์ตจาก scan)
- `data/portfolios/{A,B,C}.json` — **3 พอร์ตแยกไฟล์ (v6.8.0)** single source ต่อพอร์ต (name/targets/holdings/cash/off_plan/lh_triggers/meta). A = ของเดิม, B/C โครงหุ้น+เป้าเดียวกัน (จำนวน 0). gitignored — runtime ส่วนตัว (privacy). _legacy `data/portfolio.json` archived 2026-06-25 — เลิก track git (ยังอยู่ใน disk เป็น fallback A ใช้งานปกติ)_
