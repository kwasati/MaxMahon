...
- **Stack:** Python + SETSMART API (primary aggregate) + thaifin (history) + yahooquery (DPS events + 52w/capex/IE) + Anthropic SDK (claude-opus-5)
...
- **Schedule:** APScheduler ใน server — **3 cron jobs**: (1) อาทิตย์ 09:00 Asia/Bangkok → weekly scan 933 หุ้น, (2) ทุกวัน 19:00 Asia/Bangkok → daily price refresh (watchlist + PASS + พอร์ตทุกตัว → `data/price_cache/{sym}.json`; v6.8.0 ใส่ `misfire_grace_time=3600` + `coalesce` + catch-up ตอน boot กัน 19:00 ไม่ยิง), (3) อาทิตย์ 06:00 Asia/Bangkok → weekly dividend refresh (`set.or.th` DPS cache for full universe, 3hr before weekly scan). **log → `data/logs/server.log`** (rotating; เดิม uvicorn warning-level กลืน INFO หมด)
...
- `scripts/portfolio_state.py` — **พอร์ตจริง (v6.7.0 redesign + v6.8.0 multi-portfolio + onetable-flow)**: load/save/`build_state()`/`compute_buy_plan()` รับ `portfolio_id` (A/B/C, default A + fallback `portfolio.json` เดิมกัน server พังก่อน migrate) อ่านจาก `data/portfolios/{id}.json` · `build_state` คืน positions/cash/summary + **`plan`** (แผนซื้อ greedy lot-by-lot จากเงินสดส่วนเกินเป้า, ไม่มีช่องกรอกเงินเติมแยกแล้ว) + `price_as_of` (newest mtime ใน price_cache) + `name`. Used by `/api/portfolio/{state,holdings}` (ทุกตัวรับ `?pf=`). คนละ feature กับ portfolio_builder (นั่น = สร้างพอร์ตจาก scan)
- `data/portfolios/{A,B,C}.json` — **3 พอร์ตแยกไฟล์ (v6.8.0)** single source ต่อพอร์ต (name/targets/holdings/cash/meta). A = ของเดิม, B/C โครงหุ้น+เป้าเดียวกัน (จำนวน 0). gitignored — runtime ส่วนตัว (privacy). _legacy `data/portfolio.json` archived 2026-06-25 — เลิก track git (ยังอยู่ใน disk เป็น fallback A ใช้งานปกติ)_
- `scripts/migrate_portfolio_to_separate.py` — one-shot ย้าย `portfolio.json` → `portfolios/A.json` + seed B/C (copy mode ไม่ย้ายไฟล์เดิม, idempotent)
- `scripts/daily_price_refresh.py` — daily 19:00 price refresh for watchlist + PASS + **portfolio holdings ทุกพอร์ต** (v6.8.0 — `_load_symbols()` รวม symbols จาก `data/portfolios/*.json` ทุกไฟล์, fallback `portfolio.json` ถ้ายังไม่ migrate). job ตั้ง `misfire_grace_time=3600` + `coalesce` + catch-up ตอน boot (ราคาเก่า >18h ดึงเอง) กัน 19:00 ไม่ยิง. **SETSMART EOD bulk = primary** (1 request → 875+ CS symbols, strip `.BK`), yahooquery batch fallback, sequential retry rounds. Writes `data/price_cache/{sym}.json` `{symbol, price, fetched_at, source}`. log → `data/logs/server.log`
